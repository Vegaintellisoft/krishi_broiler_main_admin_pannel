...
const IssuedMedicine = () => {
    // --- Permissions ---
    const { getPermissions } = useAuth();
    const { medicineIssued } = getPermissions(); // Assuming permission key is 'medicineIssued'
    ...
    const fetchData = async () => {
        ...
        const IssueMedRes = await axios.get("broiler/issue-medicine/getAll");
        console.log(IssueMedRes)

        if(IssueMedRes.data.status) {
            setData(IssueMedRes.data.data);
        } else {
            setData([]);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        setTimeout(() => {
            if (editId) {
                ...
                Swal.fire({ icon: "success", title: "Updated!", text: "Issued Medicine updated successfully.", timer: 1500, showConfirmButton: false });
            } else {
                setData(prev => [...prev, { ...formData, id: Date.now() }]);
                Swal.fire({ icon: "success", title: "Saved!", text: "Issued Medicine added successfully.", timer: 1500, showConfirmButton: false });
            }
            closeModal();
    ...
    const handleExport = () => {
        ExcelExport(data, "Issued_Medicine.xlsx");
    };

    // --- Filter & Pagination ---
    const filteredData = data.filter(item =>
        item.farmer_no.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.branch.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.doc_no.toLowerCase().includes(searchQuery.toLowerCase())
    );
        ...
        <div className="rounded-lg shadow flex-1">
                    ...
                    <table className="w-full text-sm text-left">
                        <thead className="text-black font-poppins font-semibold border-b">
                            <tr>
                                ...
                                <th className="p-4 text-center">DC</th>
                                {(medicineIssued?.edit || medicineIssued?.delete) && <th className="p-4 text-center">Actions</th>}
                            </tr>
                        </thead>
                        <tbody className="divide-y font-poppins">
                            {paginatedData.length > 0 ? paginatedData.map((item, index) => (
                                <tr key={item.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="p-4 opacity-65">{formatDate(item.doc_date)}</td>
                                    <td className="p-4 opacity-65">{item.branch}</td>
                                    <td className="p-4 opacity-65">{item.farmer_no}</td>
                                    {/* <td className="p-4 opacity-65">{item.batchNo}</td> */}
                                    <td className="p-4 opacity-65">
                                        {item.medicine_issue && item.medicine_issue.length > 0 ? (
                                            item.medicine_issue.map((m, idx) => (
                                            <div key={idx} className="mb-1.5 text-xs">
                                                <p className='text-xs font-bold'>{m.material_name}</p>
                                                <p className='text-[10px] font-medium'>{m.issue_quantity} {m.uom}</p>
                                            </div>
                                            ))
                                        ) : (
                                    ...
                                    {(medicineIssued?.edit || medicineIssued?.delete) && (
                                        <td className="p-4 flex justify-center gap-4">
                                            {medicineIssued?.edit && <button onClick={() => openModal(item)} className="hover:text-orange-500"><FiEdit2 size={18} /></button>}
                                            {medicineIssued?.delete && <button onClick={() => handleDelete(item.id)} className="hover:text-red-500"><RiDeleteBin6Line size={18} /></button>}
                                        </td>
                                    )}
                                </tr>
                            )) : (
            ...
            {modalIsOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50 backdrop-blur-sm">
                    <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl mx-4 overflow-hidden font-poppins">

                        {/* Modal Header */}
                        <div className="flex justify-between items-center p-5 border-b bg-gray-50">
                            <h2 className="text-lg font-bold text-gray-800">{editId ? "Edit Issued Medicine" : "Add Issued Medicine"}</h2>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-6">
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        // Match selected name to key or directly use value if editing
                                ...
                                {/* Row 2: Batch No, Bird Stock, Age */}
                                <div>
                                    <label className="block text-xs font-bold text-gray-700 mb-1">Batch No <span className="text-red-500">*</span></label>
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-gray-700 mb-1">Age <span className="text-red-500">*</span></label>
                                    <input type="text" name="age" value={formData.age} readOnly className="w-full p-2.5 bg-gray-100 border rounded-lg text-sm text-gray-600 focus:outline-none" placeholder="Auto-filled" />
                                </div>

                                {/* Row 3: Material, Transfer Quantity */}
                                <div>
                                    <label className="block text-xs font-bold text-gray-700 mb-1">Material <span className="text-red-500">*</span></label>
                                    <select
                                        name="material"
