...
const FcrGrade = () => {
    ...
    const fetchMaster = async () => {
        ...
        try {
            ...
            console.log("Error fetching fcr grade master:", err)
        } finally {
            setIsLoading(false)
        }
    }
    ...
    const handleExport = () => {
        ...
        ExcelExport(data, "fcr_grade_master.xlsx");
    };
    ...
    const filteredData = data.filter(
        (item) =>
            ...
            item.zsel.toLowerCase().includes(searchQuery.toLowerCase()) ||
            item.zerc.toLowerCase().includes(searchQuery.toLowerCase()) ||
            item.zerc1.toLowerCase().includes(searchQuery.toLowerCase()) ||
            item.zgrade.toLowerCase().includes(searchQuery.toLowerCase()) 
    );
        ...
        <div className="rounded-lg shadow flex-1">
            ...
            <div className="bg-[#F9F9FC] h-screen relative">
                <div className="space-y-4 pt-3 px-6 font-poppins">
                    <h1 className="text-xl font-bold text-gray-900">FCR Grade Master</h1>
                    <div className="flex items-center gap-x-2 text-sm text-gray-500">
                        <Link to="/" className="text-orange-500">
                            Masters
                        </Link>
                        <RiArrowUpSFill className="rotate-90" size={20} />
                        <span>FCR Grade Master</span>
                    </div>
                            ...
                            {paginatedData.length > 0 ? (
                                paginatedData.map((item, index) => (
                                    <tr key={item.id} className="hover:bg-gray-50 text-center">
                                        ...
                                        <td className="p-4 text-sm opacity-65">{item.zerc1}</td>
                                        <td className="p-4 text-sm opacity-65">{item.zgrade}</td>
