...
const MortalityIncenPlant = () => {
    ...
    const fetchMaster = async () => {
        setIsLoading(true)
        try {
            const { data } = await axios.get("broiler/master/getAll/mortality_incentive_plant");
            setData(data.data);
        }
        catch (err) {
            console.log("Error fetching mortality incentive plant:", err)
        } finally {
            setIsLoading(false)
        }
    }
    ...
    const handleExport = () => {
        ...
        ExcelExport(data, "mortality_incentive_plant.xlsx");
    };
        ...
        <div className="rounded-lg shadow flex-1">
            ...
            <div className="bg-[#F9F9FC] h-screen relative">
                <div className="space-y-4 pt-3 px-6 font-poppins">
                    <h1 className="text-xl font-bold text-gray-900">Mortality Incentive Plant</h1>
                    <div className="flex items-center gap-x-2 text-sm text-gray-500">
                        <Link to="/" className="text-orange-500">
                            Masters
                        </Link>
                        <RiArrowUpSFill className="rotate-90" size={20} />
                        <span>Mortality Incentive Plant</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex space-x-4">
                            <div className="relative">
                                ...
                                <input
                                    type="search"
                                    placeholder="Search Mortality Incen..."
                                    value={searchQuery}
...
export default MortalityIncenPlant;