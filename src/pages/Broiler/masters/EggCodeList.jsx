...
const EggCodeList = () => {
    ...
    const fetchMaster = async () => {
        ...
        try {
            ...
            console.log("Error fetching egg code list:", err)
        } finally {
    ...
    const handleExport = () => {
        ...
        ExcelExport(data, "egg_code_list.xlsx");
    };
    ...
    const filteredData = data.filter(
        (item) =>
            ...
            item.matnr.toLowerCase().includes(searchQuery.toLowerCase()) ||
            item.maktx.toLowerCase().includes(searchQuery.toLowerCase()) 
    );
                            ...
                            {paginatedData.length > 0 ? (
                                paginatedData.map((item, index) => (
                                    <tr key={item.id} className="hover:bg-gray-50 text-center">
                                        ...
                                        <td className="p-4 text-sm opacity-65">{item.matnr}</td>
                                        <td className="p-4 text-sm opacity-65">{item.maktx}</td>
                                    </tr>
                                ))
                            ) : (
                                <tr>
                                    <td
                                        colSpan="4" // Adjusted to 4 since only 4 columns remain (S.No, Mandt, Werks, zzline)
