...
const MortalityDeducPlant = () => {
    ...
    const fetchMaster = async () => {
        setIsLoading(true)
        try {
            const { data } = await axios.get("broiler/master/getAll/mortality_deduction_plant");
            setData(data.data);
        }
        catch (err) {
            console.log("Error fetching mortality deduction plant:", err)
        } finally {
            setIsLoading(false)
        }
    }
    ...
    const handleExport = () => {
        ...
        ExcelExport(data, "mortality_deduction_plant.xlsx");
    };
        ...
        <div className="rounded-lg shadow flex-1">
            ...
            <div className="bg-[#F9F9FC] h-screen relative">
                <div className="space-y-4 pt-3 px-6 font-poppins">
                    <h1 className="text-xl font-bold text-gray-900">Mortality Deduction Plant</h1>
                    <div className="flex items-center gap-x-2 text-sm text-gray-500">
                        <Link to="/" className="text-orange-500">
                            Masters
                        </Link>
                        <RiArrowUpSFill className="rotate-90" size={20} />
                        <span>Mortality Deduction Plant</span>
                    </div>
...
export default MortalityDeducPlant;