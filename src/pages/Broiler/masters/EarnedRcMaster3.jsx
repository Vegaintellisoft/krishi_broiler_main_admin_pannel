...
const EarnedRcMaster3 = () => {
    ...
    const fetchMaster = async () => {
        setIsLoading(true)
        try {
            const { data } = await axios.get("broiler/master/getAll/earned_rc_master3");
            setData(data.data);
        }
        catch (err) {
            console.log("Error fetching Earned RC Master-3:", err)
        } finally {
            setIsLoading(false)
        }
    }
    ...
    const handleExport = () => {
        ...
        ExcelExport(data, "earned_rc_master3.xlsx");
    };
    ...
    const filteredData = data.filter(
        (item) =>
            ...
            item.znewGc.toLowerCase().includes(searchQuery.toLowerCase()) 
    );
        ...
        <div className="rounded-lg shadow flex-1">
            ...
            <div className="bg-[#F9F9FC] h-screen relative">
                <div className="space-y-4 pt-3 px-6 font-poppins">
                    <h1 className="text-xl font-bold text-gray-900">Earned RC Master-3</h1>
                    <div className="flex items-center gap-x-2 text-sm text-gray-500">
                        <Link to="/" className="text-orange-500">
                            Masters
                        </Link>
                        <RiArrowUpSFill className="rotate-90" size={20} />
                        <span>Earned RC Master-3</span>
                    </div>
...
export default EarnedRcMaster3;