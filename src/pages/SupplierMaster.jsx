...
const SupplierMaster = () => {
  ...
  const fetchSupplierMaster = async () => {
    setIsLoading(true)
    try {
      const { data } = await axios.get("/supplier/getAll");
      setData(data.data);
    }
    catch (err) {
      console.log("Error fetching supplier master:", err)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (updatedSupplierId) {
        response = await axios.put(`/supplier/update/${updatedSupplierId}`, formData);
      } else {
        response = await axios.post("/supplier/add", formData);
      }
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const { data } = await axios.delete(`/supplier/delete/${id}`);
          if (data.status) {
            Swal.fire({
              toast: true,
              position: "top-end",
  ...
  const handleExport = () => {
    ExcelExport(data, "Supplier_Master.xlsx");
  };
    ...
    <div className={` rounded-lg shadow flex-1 `}>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
          ...
          >
            <div className="flex justify-between border-b py-2">
              <h1 className="header text-lg font-semibold">Add Supplier  Master</h1>
              <button
      ...
      <div className={` bg-[#F9F9FC] h-screen relative`}>
        ...
        <div className=" space-y-4 pt-3 px-6 font-poppins">
          <h1 className="text-xl font-bold text-gray-900">Supplier Masters</h1>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className='space-x-5'>
              ...
              {
                supplierMaster?.add && <button className="px-4 py-2 bg-orange-500 text-white border rounded-lg  hover:text-white" onClick={openModal}>
          ...
          <table className={`w-full`}>
            <thead className='font-poppins font-semibold'>
              <tr className="border-b">
                ...
                {
                  (!supplierMaster?.edit && !supplierMaster?.delete) ? "" : <th className="p-4 text-center text-sm text-black">Actions</th>
                }
            ...
            <tbody className="divide-y font-poppins">
              {paginatedData.map((item, index) => (
                <tr key={index} className="hover:bg-gray-50 text-center">
                  ...
                  <td className="p-4 text-sm opacity-65">{!item.supplier_id?"<<null>>":item.supplier_id}</td>
                  ...
                  {
                    (!supplierMaster?.edit && !supplierMaster?.delete) ?
                      ""
                      :
                      <td className="p-4 text-right space-x-5 flex justify-center opacity-65">
                        {
                          supplierMaster?.edit && <button onClick={() => handleEdit(item)} className=" hover:text-gray-700">
                            <FiEdit2 size={20} />
                          </button>
                        }

                        {
                          supplierMaster?.delete && <button onClick={() => handleDelete(item.id)} className=" hover:text-gray-700">
